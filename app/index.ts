...
// Only initialize neon if the URL is valid to avoid immediate crash
const sql = isValidDbUrl 
  ? neon(dbUrl) 
  : ((...args: any[]) => { 
      console.error('Database operation attempted but DATABASE_URL is invalid.');
...
async function callAIGateway(prompt: any, isImage = false) {
  ...
  if (isImage && !prompt.config) {
    prompt.config = { imageConfig: { aspectRatio: "1:1" } };
  }
  ...
  // Gateway URL이 있으면 거기로, 없으면 직통으로 (보안을 위해 서버에서만 호출)
